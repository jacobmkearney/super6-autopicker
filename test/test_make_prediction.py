...
def test_correct_score_prediction(mock_load_json):
    # Given valid JSON data for predictions
    mock_load_json.side_effect = [
        [{"home_team": "Team A", "away_team": "Team B", "predictions": [{"name": "Team A", "price": 2.0}, {"name": "draw", "price": 3.0}, {"name": "Team B", "price": 4.0}]}],
        [{"home_team": "Team A", "away_team": "Team B", "market": "totals", "bookmaker": "MockBookmaker", "predictions": [{"name": "Over", "price": 3.0, "point": 2.5}, {"name": "Under", "price": 1.1, "point": 2.5}], "most_likely": {"name": "Under", "price": 1.1, "point": 2.5}}]
    ]
    # When predict_all_matches is called
    predictions = predict_all_matches()
    # Then it returns the expected scores and probability
    assert len(predictions) == 1
    assert predictions[0]["Team A"] == 1 
    assert predictions[0]["Team B"] == 0  
    assert predictions[0]["probability"] is not None


@mock.patch('src.super6_auto_picker.make_prediction.load_json')
def test_handling_missing_data(mock_load_json):
    # Given JSON data with missing predictions
    mock_load_json.side_effect = [
    ...
    # When predict_all_matches is called
    predictions = predict_all_matches()
    # Then it returns None for scores and probability due to missing data
    assert len(predictions) == 1
    assert predictions[0]["Team A"] is None
    assert predictions[0]["Team B"] is None
    assert predictions[0]["probability"] is None


@mock.patch('src.super6_auto_picker.make_prediction.load_json')
def test_edge_cases(mock_load_json):
    # Given JSON data with edge case prices
    mock_load_json.side_effect = [
    ...
    # When predict_all_matches is called
    predictions = predict_all_matches()
    # Then it returns None for scores and probability due to invalid prices
    assert len(predictions) == 1
    assert predictions[0]["Team A"] is None
    assert predictions[0]["Team B"] is None
    assert predictions[0]["probability"] is None
...
def test_error_handling(mock_load_json):
    # Given JSON data with missing keys
    mock_load_json.side_effect = [
    ...
    # When predict_all_matches is called
    predictions = predict_all_matches()
    # Then it returns None for scores and probability due to missing keys
    assert len(predictions) == 1
    assert predictions[0]["Team A"] is None
    assert predictions[0]["Team B"] is None
    assert predictions[0]["probability"] is None